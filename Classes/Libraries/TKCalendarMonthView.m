...
- (CGRect) rectForCellAtIndex:(int)index{
    ...
    return CGRectMake(col*32, row*36+6, 32, 36);
}
...
- (void) drawRect:(CGRect)rect {
	...
	if(today > 0){
       ...
       // r=CGRectMake(0, 0, 32, 32);
        ...
        [[UIImage imageNamed:@"Active-day-cell.png"] drawInRect:r];
		//[[UIImage imageWithContentsOfFile:TKBUNDLE(@"TapkuLibrary.bundle/Images/calendar/Month Calendar Today Tile.png")] drawInRect:r];
	}
	
	int index = 0;
	
	UIFont *font = [UIFont fontWithName:@"HelenBgCondensed-Bold" size:dateFontSize];
	UIFont *font2 =[UIFont fontWithName:@"HelenBgCondensed-Bold" size:dotFontSize];
...
- (void) selectDay:(int)day{
	...
	if(day == today){
		...
		//self.dot.shadowOffset = CGSizeMake(0, 1);
         self.selectedImageView.image=[UIImage imageNamed:@"day-cell.png"];
    ...
    self.currentDay.font=[UIFont fontWithName:@"HelenBgCondensed-Bold" size:26];
...
- (void) reactToTouch:(UITouch*)touch down:(BOOL)down{
	...
	int column = p.x / 32, row = p.y / 36;
	int day = 1, portion = 0;
	
	if(row == (int) (self.bounds.size.height / 36)) row --;
...
- (UILabel *) currentDay{
	if(currentDay==nil){
        ...
        currentDay.font=[UIFont fontWithName:@"HelenBgCondensed-Bold" size:dateFontSize];
		currentDay.text = @"1";
...
- (UILabel *) dot{
	if(dot==nil){
		...
		dot.font = [UIFont fontWithName:@"HelenBgCondensed-Bold" size:dotFontSize];
...
- (id) initWithSundayAsFirst:(BOOL)s{
	...
	[currentTile setTarget:self action:@selector(tile:)];
	CGRect r = CGRectMake(0, 0, self.tileBox.bounds.size.width-53, self.tileBox.bounds.size.height + self.tileBox.frame.origin.y);
	...
	[currentTile retain];
	self.topBackground.frame=CGRectMake(0, -3, 272, 60);
	[self addSubview:self.topBackground];
	[self.tileBox addSubview:currentTile];
	[self addSubview:self.tileBox];
	
	NSDate *date = [NSDate date];
    self.monthYear.font=[UIFont fontWithName:@"HelenBgCondensed-Bold" size:26];
	...
	for(NSString *s in ar){
        ...
        UIImageView *img=[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"day-of-week-cell.png"]];
        img.frame=CGRectMake(32*i, 31, 40, 15);
		UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(2, 2, 30, 13)];
        label.backgroundColor=[UIColor clearColor];
        //label.frame.origin.x=label.frame.origin.x-5;
...
//        label.frame = frame;
        ...
        //[label setAutoresizingMask:UIViewContentModeTop];
        
		[self addSubview:img];
         [img addSubview:label];
        label.font=[UIFont fontWithName:@"HelenBgCondensed-Bold" size:12];
		label.text = s;
		label.textAlignment = UITextAlignmentCenter;
		//label.shadowColor = [UIColor whiteColor];
		//label.shadowOffset = CGSizeMake(0, 1);
		
		//label.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"day-of-week-cell.png"]];
...
- (void) changeMonthAnimation:(UIView*)sender{
	...
	if(isNext){
		overlap = [newTile.monthDate isEqualToDate:[dates objectAtIndex:0]] ? 0 : 40;
	}else{
		overlap = [currentTile.monthDate compare:[dates lastObject]] !=  NSOrderedDescending ? 40 : 0;
	}
...
- (void) changeMonthNew{
	...
	NSDate *newDate = [self dateForMonthChangeNew];
	if ([delegate respondsToSelector:@selector(calendarMonthView:monthShouldChange:animated:)] && ![delegate calendarMonthView:self monthShouldChange:newDate animated:YES] ) 
		return;
...
- (void) selectDate:(NSDate*)date{
	...
	if([month isEqualToDate:[currentTile monthDate]]){
		...
		currentTile = newTile;
		[self.tileBox addSubview:currentTile];
		self.tileBox.frame = CGRectMake(0, 40, newTile.frame.size.width, newTile.frame.size.height);
		self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.bounds.size.width, self.tileBox.frame.size.height+self.tileBox.frame.origin.y);
...
- (UIImageView *) topBackground{
	if(topBackground==nil){
        topBackground=[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"monthBg.png"]];
...
- (UILabel *) monthYear{
	if(monthYear==nil){
		monthYear = [[UILabel alloc] initWithFrame:CGRectMake(-25, 0, self.tileBox.frame.size.width, 32)];
		
		monthYear.textAlignment = UITextAlignmentCenter;
		monthYear.backgroundColor = [UIColor clearColor];
		monthYear.font = [UIFont fontWithName:@"HelenBgCondensed-Bold" size:26];
        monthYear.textColor=[UIColor whiteColor];
...
- (UIScrollView *) tileBox{
	if(tileBox==nil){
		tileBox = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 40, 272, currentTile.frame.size.height)];
	}
	return tileBox;
}
- (UIImageView *) shadow{
